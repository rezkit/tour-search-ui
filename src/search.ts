...
export const SearchRequest = z
  .object({
    ...
    fh: z
      .record(
        z.string(),
        z.union([z.string(), z.number(), z.boolean(), z.null(), StringArray])
      )
      .optional(),
    fd: z
      .record(
  ...
  .refine((params) => !(params.q && params.qs), {
    message: '`q` and `qs` are mutually exclusive',
    path: ['q', 'qs'],
  })
